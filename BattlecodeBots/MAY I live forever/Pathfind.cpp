...
bool Pathfind::MoveOptimized(units::Robot & robot, MapLocation & destination)
{
	// TRY Is there a flowChart to the destination? Use it!
	if (!hasInit) {
		Init();
	}
	int destX = destination.X();
	...
	// TRY Can I fuzzy move to the destination within x turns? If so return FuzzyMove!
	if (CanFuzzyReach(mapLoc, destination)) {
		return MoveFuzzy(robot, mapLoc.DirectionTo(destination));
	}
	// TRY If I can't fuzzy move there, can I fuzzy get there from a close flowChart that has already been generated? Move towards the close flowChart!
	// TRY No hope... Generate a new flowChart to get there!
	GenerateFlowPath(destX, destY);
	flowChart = &flowCharts[destXY];
	short currX = static_cast<short>(mapLoc.X());
	short currY = static_cast<short>(mapLoc.Y());
...
bool Pathfind::CanFuzzyReach(MapLocation & origin, MapLocation & destination)
{
	PlanetMap map = GameController::PlanetMap(origin.Planet());
	MapLocation current = origin;
	for (int i = 0; i < origin.TilesTo(destination) + 5; i++) {
		if (current == destination) return true;
		bc_Direction dir = current.DirectionTo(destination);
		MapLocation probe = MapLocation::Neighbor(current, dir);
		if (map.IsPassableTerrain(probe)) {
			current == probe;
			continue;
		}
		probe = MapLocation::Neighbor(current, bc_Direction_rotate_right(dir));
		if (map.IsPassableTerrain(probe)) {
			current == probe;
			continue;
		}
		probe = MapLocation::Neighbor(current, bc_Direction_rotate_left(dir));
		if (map.IsPassableTerrain(probe)) {
			current == probe;
			continue;
		}
