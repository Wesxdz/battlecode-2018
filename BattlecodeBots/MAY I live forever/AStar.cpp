...
std::vector<bc_Direction> AStar::PathFind(const int & xStart, const int & yStart, const int & xFinish, const int & yFinish, bool earth)
{
	...
	uint8_t* map = new uint8_t[m_mapHSize * m_mapVSize]();
	
	uint8_t* closedNodesMap = new uint8_t[m_mapHSize * m_mapVSize]();
	
	uint8_t* openNodesMap = new uint8_t[m_mapHSize * m_mapVSize]();
	
	uint8_t* directionMap = new uint8_t[m_mapHSize * m_mapVSize]();
	...
	if (hNode == nullptr)
	{
		std::cout << "hNode is null" << std::endl;
	}
	...
	while (!pq[pqIndex].empty())
	{
		...
		for (i = 0; i < dir; i++)
		{
			...
			if (!(xdx<0 || xdx > m_mapHSize - 1 || ydy<0 || ydy > m_mapVSize - 1 || map[xdx * m_mapVSize + ydy] == 1 || closedNodesMap[xdx * m_mapVSize + ydy] == 1))
			{
				...
				if (vNode == nullptr)
				{
					std::cout << "vNode is null" << std::endl;
				}
				vNode->NextLevel(i);
